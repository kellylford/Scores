if pitch_plays:
    print(f'\\nExamining pitch play structure (first of {len(pitch_plays)} pitch plays):')
    print(json.dumps(pitch_plays[0], indent=2))
else:
    print('No pitch plays found')    if pitch_plays:
        print(f'\\nExamining pitch play structure (first of {len(pitch_plays)} pitch plays):')
        print(json.dumps(pitch_plays[0], indent=2))
    else:
        print('No pitch plays found')import sys
sys.path.insert(0, '.')if pitch_plays:
    print(f'\\nExamining pitch play structure (first of {len(pitch_plays)} pitch plays):')
    print(json.dumps(pitch_plays[0], indent=2))
else:
    print('No pitch plays found')    if pitch_plays:
...
def extract_pitch_data():
    ...
    for play in plays:
        ...
        # Extract pitch data
            ...
            velocity = play.get('pitchVelocity', {}).get('value') if play.get('pitchVelocity') else None
            ...
            # Look for details in alternativeText or other fields
            details_text = play.get('alternativeText', '') or play.get('text', '')
            ...
            pitch_data = {
                ...
                'participants': len(play.get('participants', [])),
